...
int main() {
    ...
    //"sicilno, isim, satis" yazı formatında olan verileri değişkenlere atıyoruz
    while (fscanf(file,"%d, %50[^,], %d\n",&sicil_no, isim, &satis) !=EOF) {
        if (satis>max_satis) {
    ...
    //Yine "sicilno, isim, satis" yazı formatındaki verileri değişkenlere atıyoruz
    while (fscanf(file,"%d, %50[^,], %d",&sicil_no, isim, &satis) !=EOF) {
